...
def zezinho():
    ...
    # Extract the 'timestamp' field and convert it to ISO format.
    serialized_times = [entity["timestamp"].isoformat() for entity in times if "timestamp" in entity]

    # Return the times as JSON.
    return jsonify({"times": serialized_times})
    # Return the times as a JSON response.
